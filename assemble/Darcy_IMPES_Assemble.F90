...
subroutine darcy_trans_leaching_solve_generic_prog_single_sfield_sub(di,f,p,isub)
   ...
   type(scalar_field) :: shared_rhs, shared_lhs,field_pmesh,dtheta_dt, rhs
   ...
   call zero(di%rhs)
   call zero(di%rhs_full)
   call allocate(matrix, di%sparsity_pmesh_pmesh)
   call allocate(rhs, di%pressure_mesh)
   call zero(matrix)
   call zero(rhs)
   ...
   if (di%current_time>=1.0e-10) then
      matrix%val=isub_e*di%matrix%val + (1-isub_e)*di%lcsub%old_sub_adv_diff(f)%val
      rhs%val=isub_e*di%rhs%val + (1-isub_e)*di%lcsub%old_sub_rhs(f)%val     
   end if
   
   if (isub==di%lcsub%number_subcycle) then
      di%lcsub%old_sub_adv_diff(f)%val=di%matrix%val
      di%lcsub%old_sub_rhs(f)%val=di%rhs%val
   end if
   
   di%matrix%val=matrix%val
   di%rhs%val=rhs%val
...
subroutine calculate_leach_prog_sfield_subcycle_terms(di)
  ...
  do i=1, di%number_phase    
     if (di%MIM_options%have_MIM(i)) then
       ...
       call scale(di%lcsub%sub_lht_im(i),di%MIM_options%immobile_saturation(i)%ptr)
    else
       call set(di%lcsub%sub_lht(i),di%porosity_pmesh)
...
subroutine leaching_prog_sfield_subcycle_finalize(di)
     ...
     f=size(di%lcsub%old_sub_adv_diff)
     do i=1, f
